...
class Tool extends Component {
    ...
    onFileSelected = async (e) => {
        ...
        data.append("pdfFile", fileList);
        let response="";
        if (fileList.size > 0) {     
             response = await this.props.store.api
                .post("http://localhost:3081/api/ai/personal/embedding", data)
            this.load = false;
            this.status ="Embeded";
        }
        
        if (response.data) {
            this.output =  this.checkOutput(response.data)        
            this.answer = this.checkOutput(response.data)       
        }
    ...
    onGenerateClick = async () => {
        try {
            ...
            var data = new FormData();
            data.append("query", postObj["query"]);
            let context = await this.props.store.api
                .post("http://localhost:3081/api/ai/personal/context", data)
            postObj['context'] =  context['data'][1]
            ...
            if (response.data.output) {
                this.gpt_res = this.checkOutput(response.data.output)
                this.output = "GPT response:" + "\n" + this.checkOutput(response.data.output) +"\n\n"+"Context Items Applied:\n" +context['data'][0]
                this.answer = this.checkOutput(response.data.output)
            }
    ...
    render() {
        ...
        return (
            <>
                <Helmet>
                    <title>{`${this.tool.title} Tool - LumicAI`}</title>
                </Helmet>
                <Header
                        ...
                        <Col span="12">
                            ...
                            {this.prompts.map((prompt, index) =>
                                ...
                                >
                                    ...
                                    <div className="md:flex">
                                        <Loader active={this.load} className="w-10 h-10"/>  
                                        <input type="file"   accept='.doc, .docx , application/pdf'  hidden={this.tool.title!="Semantic Search"} onChange={this.onFileSelected}></input>
